"""
===============================================================================
🛡️ NETRAM AI — LOCAL INFERENCE ENGINE & WEBSOCKET SERVER
===============================================================================
Native High-Throughput WebSocket Deepfake & Presentation Attack Inference Engine.
Serves the Chrome Extension by running the complete real-time multi-modal pipeline:
...
- Presentation Attack / Smartphone Replay Detector (YOLOv8 + Bezel + Moiré)
- Snapchat / Instagram / Social Media AR Beauty Filter Detector
...
Runs purely in the terminal on ws://127.0.0.1:8765.
...
class DeepfakeExtensionServer:
    """
    Production-grade WebSocket deepfake and presentation attack inference server.
    """
    ...
    def __init__(self, host: str = "0.0.0.0", port: int = 8765):
        ...
        logger.info("Initializing Pipeline Orchestrator with all trained models & neural weights...")
    ...
    def process_participant_frame(
        ...
        # Stage 4: Multi-Modal Neural Branches (Spatial ViT + Frequency CNN + AASIST + Phone Replay + AR Filters)
    ...
    def http_process_request(self, connection, request):
        """Handles HTTP health checks and status queries."""
        ...
        if path in ("/", "/health", "/ping", "/status"):
            payload = json.dumps({
                "status": "healthy",
                "service": "Netram AI Real-Time Deepfake & Presentation Attack Engine",
                "version": "2.1.0",
                "protocols": ["HTTP/1.1", "WebSocket (WSS)"],
                "active_clients": len(self.clients),
                "message": "Netram AI engine is online and ready."
            }, indent=2)
    ...
    async def start(self):
        logger.info(f"🚀 Starting Netram AI Engine on ws://{self.host}:{self.port}")
...
def run_server(host: str = "0.0.0.0", port: int = 8765):
    ...
    try:
        ...
        logger.info("Netram AI local inference engine stopped.")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Netram AI Deepfake & Presentation Attack Inference Engine")
    ...
    print(f"{CYAN}{BOLD}   🛡️  NETRAM AI — REAL-TIME DEEPFAKE & PRESENTATION ATTACK ENGINE{RESET}")
    print(f"{CYAN}{BOLD}" + "=" * 75 + f"{RESET}\n")

    print(f"  {BOLD}• WebSocket Endpoint:{RESET}    {GREEN}ws://127.0.0.1:{args.port}{RESET}")
    print(f"  {BOLD}• HTTP Health Check:{RESET}     {GREEN}http://127.0.0.1:{args.port}/health{RESET}")
    print(f"  {BOLD}• Active Models:{RESET}         {CYAN}Spatial ViT · 2D DCT CNN · Bi-GRU · Phone Replay · AR Filters · AASIST{RESET}")
